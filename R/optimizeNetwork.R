optimizeNetwork = function( observations, 
                            ...
                            criterion = "MUKV",
                            plotOptim = TRUE, nGridCells, nTry, nr_iterations, formulaString, ...)
  {
# list here all cases when to stop
  if (length(method)==0) stop(cat("No 'method' informed..."))
  if (length(method)>0){
    if (method != "spcov" & method != "ssa" & method != "manual") stop(cat("No 'method' required ..."))
  ...
  if (length(action)==0) stop(cat("No 'action' defined ... write 'add' or 'del'."))
  if (length(action)>0){
    if (action != "add" & action != "del") stop(cat("No relevant 'action' defined ... write 'add' or 'del'."))
    if (action == "add"){
  ...
  if (length(nDiff) == 0 | nDiff<=0) stop(cat("nDiff is not well defined", nDiff))
...
# starts here
  if (method == "ssa"){
    ...
    nr_iterations = 50
    return(ssaOptim(observations, predGrid, candidates, action, nDiff, model, 
           nr_iterations, plotOptim, formulaString, ...))
    }
  if (method == "spcov"){
    if (action == "add"){
